...
import { GetPopularListResponeDto } from 'apis/response/search'
...
export default function Main() {
        ...
        return (
            <div id='main-top-wrapper'>
                <div className='main-top-container'>
                    <div className='main-top-title'>{'SUHO Board에서 \n 다양한 이야기를 나눠보세요'}</div>
    ...
    const MainBottom = () => {
        ...
        const getPopularListResponse = (responseBody: GetPopularListResponeDto | ResponseDto | null) => {
            if(!responseBody) return;
            const {code} = responseBody;
            if(code === 'DBE') alert('데이터베이스 오류입니다.');
            if(code !== 'SU') return;

            const {popularWordList} = responseBody as GetPopularListResponeDto;
